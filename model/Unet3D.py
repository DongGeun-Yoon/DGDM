...
class SpatialLinearAttention(nn.Module):
    def __init__(self, dim, heads = 4, dim_head = 32):
        ...
        self.cond = nn.Sequential(
            nn.Conv3d(64, dim, kernel_size=(1,1,1)),
            nn.GroupNorm(8, dim),
            nn.GELU(),
            nn.Conv3d(dim, dim, kernel_size=(1,1,1))
        )
    ...
    def forward(self, x, context=None):
        ...
        q, k, v = rearrange_many((q,k,v), 'b (h c) x y -> b h c (x y)', h = self.heads)
...
class Unet3D(nn.Module):
    def __init__(
        ...
        self.final_conv = nn.Sequential(
            block_klass(dim * 2 + 64, dim),
            nn.Conv3d(dim, out_dim, 1)
        )

    def forward(self, x, timesteps, context=None, focus_present_mask = None, prob_focus_present=0., train=False):
        ...
        assert not (self.has_cond and not exists(context)), 'cond must be passed in if cond_dim specified'
        ...
        for block1, block2, spatial_attn, temporal_attn, downsample in self.downs:
            x = block1(x, t)
            x = block2(x, t)
            x = spatial_attn(x, context=context)
            c.append(context)
        ...
        for block1, block2, spatial_attn, temporal_attn, upsample in self.ups:
            ...
            x = block1(x, t)
            x = block2(x, t)
            x = spatial_attn(x, context=c.pop())
            x = temporal_attn(x, pos_bias = time_rel_pos_bias, focus_present_mask = focus_present_mask)
            x = upsample(x)
            
        rr = F.interpolate(rr, scale_factor=(1,4,4)) #
        x = torch.cat((x, r, rr), dim = 1)
        if train and context is not None:
